...
function endAnim() {
  ...
  cloneArr.forEach((clone, cloneCount) => {
    ...
    tl.to(clone.shape, {
      ...
      duration: 1.2,
      fill: (c, item) =>
        parseInt(item.getAttribute("r")) == baubleRadius
          ? clone.fill
          : checkColor(cloneCount)
          ? woodColor
          : treeColor,
...
function checkDistance(a, b) {
  return Math.hypot(a.x - b.x, a.y - b.y) < visibleArea.value * 0.45
    ? true
    : false;
}
function calc() {
